...
def _where_calc(self, domain, active_test=True):
    ...
    self.env.cr.execute("SELECT id FROM ir_module_module WHERE name='multi_search' and state='installed' limit 1")
        ...
        for domain_tuple in domain:
            ...
            if type(domain_tuple) in (list, tuple):
                if str(domain_tuple[1]) == 'ilike':
                    if domain_tuple[2]:
                        filter_value_str = u" ".join(domain_tuple[2].replace('\\', '_').split())
#                         if (filter_value_str.startswith('(') or filter_value_str.startswith('[')) and (filter_value_str.endswith(')') or filter_value_str.endswith(']')) :
#                             _logger.info('Parsing the domain filter value for multi-search condition: ' + str(domain_tuple[2]))
#                             try:
#                                 filter_value = [x.strip() for x in filter_value_str.replace('(', '').replace('[', '').replace(')', '').replace(']', '').split(',')]# eval(" ".join(filter_value_str.split()))
...
#                         el
                        if filter_value_str.startswith('{') and filter_value_str.endswith('}'):
                            ...
                            try:
                                ...
                                _logger.error('Error occured while enhancing the supplied domain filter') 
            modified_domain.append(domain_tuple)
        #_logger.info('  calculated domain filter is ' + str(modified_domain))
